...
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain (HttpSecurity http, JwtFilter jwt) throws Exception {
        ...
        }).sessionManagement (s -> s.sessionCreationPolicy (SessionCreationPolicy.STATELESS)).authorizeHttpRequests (a -> a.requestMatchers ("/api/auth/**").permitAll ().anyRequest ().authenticated ()).addFilterBefore (jwt, UsernamePasswordAuthenticationFilter.class).build ();
    }
    ...
    org.springframework.web.cors.CorsConfigurationSource corsConfigurationSource () {
        ...
        c.setAllowedOrigins(java.util.List.of(
                "http://localhost:5173",
                "https://digital-banking-frontend-snb4.onrender.com"
        ));
